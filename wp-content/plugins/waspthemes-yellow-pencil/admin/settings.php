...
function yp_welcome_screen_activate() {
	set_transient( 'yp_plugin_active_notice', true, 5 );
  set_transient( 'yp_welcome_screen_activation_redirect', true, 30 );
}
...
/* ---------------------------------------------------- */
function yp_plugin_active_notice(){
    ...
    if( get_transient( 'yp_plugin_active_notice' ) ){
        ?>
        ...
        <?php
        delete_transient( 'yp_plugin_active_notice' );
    }
		}
}

add_action( 'admin_notices', 'yp_plugin_active_notice' );
...
function welcome_screen_do_activation_redirect() {
  ...
  // Bail if no activation redirect
    if ( ! get_transient( 'yp_welcome_screen_activation_redirect' ) ) {
    return;
  }

  // Delete the redirect transient
  delete_transient( 'yp_welcome_screen_activation_redirect' );
...
function yp_welcome_screen_content(){
  ...
  <div class="wrap yp-page-welcome about-wrap">
	...
	<div class="yp-welcome-feature feature-section">
		...
		<div class="yp-column">
			<svg class="yp-img-center" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" fill="none" stroke="#000" stroke-linejoin="round" stroke-width="2"><circle cx="32" cy="32" r="30"/><circle cx="32" cy="32" r="16.506"/><path d="M20.646 20.025l-9.542-9.54M43.97 43.352l9.55 9.548m-9.516-32.224l9.545-9.542M20.676 44.006L11.13 53.55" stroke-linecap="round" stroke-miterlimit="10"/></svg>
...
function yp_option_func() {
		...
		?>
            ...
            <?php
                ...
                echo "<script>window.aceEditorBase = '".WT_PLUGIN_URL."js/ace/'; window.yp_live_styles_delete_nonce = '".wp_create_nonce('yp_live_styles_delete_nonce')."';</script>";
				...
				/* ---------------------------------------------------- */
					...
					<p class="yp-heading-text">The static external CSS file option is still in beta testing, if there is a problem please use dynamic inline CSS.</p>
					<form method="POST">
