...
{
$response = $trustindex_pm_facebook->download_noreg_details($page_details);
if ($response['success'])
{
...
$dbtable = $trustindex_pm_facebook->get_noreg_tablename();
...
dbDelta($sql);
if(!$trustindex_pm_facebook->is_noreg_table_exists())
{
...
if(!$reviews)
{
$reviews = $trustindex_pm_facebook->download_noreg_reviews($page_details, null);
...
$no_rating_text = get_option( $trustindex_pm_facebook->get_option_name('no-rating-text'));
$filter = get_option( $trustindex_pm_facebook->get_option_name('filter'), [ 'stars' => [1, 2, 3, 4, 5], 'only-ratings' => true ] );
...
$show_reviewers_photo = get_option( $trustindex_pm_facebook->get_option_name('show-reviewers-photo'), 1 );
...
$show_logos = get_option( $trustindex_pm_facebook->get_option_name('show-logos'), 1 );
$show_stars = get_option( $trustindex_pm_facebook->get_option_name('show-stars'), 1 );
if(is_null($no_rating_text) && $style_id)
{
$no_rating_text = in_array($style_id, [ 15, 19 ]) ? 1 : 0;
}
$need_to_refresh = false;
if($trustindex_pm_facebook->is_noreg_linked() && $trustindex_pm_facebook->is_ten_scale_rating_platform())
{
$fields = $wpdb->get_results('SHOW FIELDS FROM `'. $trustindex_pm_facebook->get_noreg_tablename() .'` WHERE Field = "rating"');
if($fields && isset($fields[0]) && isset($fields[0]->Type))
{
if($fields[0]->Type == 'tinyint(1)')
{
$wpdb->query('ALTER TABLE `'. $trustindex_pm_facebook->get_noreg_tablename() .'` CHANGE `rating` `rating` DECIMAL(3,1) NULL DEFAULT NULL');
...
$tmp = $trustindex_pm_facebook->download_noreg_details($page_details);
