...
$reviews = [];
if($trustindex_pm_facebook->is_noreg_linked() && $trustindex_pm_facebook->is_noreg_table_exists())
{
...
$plugin_slug = "free-facebook-reviews-and-recommendations-widgets/free-facebook-reviews-and-recommendations-widgets.php";
...
$plugin_updated = ($trustindex_pm_facebook->get_plugin_current_version() <= "7.1");
?>
<div class="ti-box">
...
<li>
<?php
$plugin_url = 'https://wordpress.org/support/plugin/' . $trustindex_pm_facebook->get_plugin_slug();
...
<?php
$dir = __DIR__ . '/../free-facebook-reviews-and-recommendations-widgets.php';
...
Options: <?php foreach ($trustindex_pm_facebook->get_option_names() as $opt_name) {
if($opt_name == "css-content")
{
continue;
}
$option = get_option($trustindex_pm_facebook->get_option_name( $opt_name ));
echo "\n\t". esc_html($opt_name) .": ";
...
CSS: <?php echo esc_html(get_option($trustindex_pm_facebook->get_option_name('css-content'))) ."\n\n"; ?>
Active Theme: <?php
