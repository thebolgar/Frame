...
'description' => TrustindexPlugin::___('Embed Facebook reviews and recommendations fast and easily into your WordPress site. Increase SEO, trust and sales using Facebook reviews.')
)
);
}
...
elseif($trustindex_pm_facebook->is_noreg_linked() && $trustindex_pm_facebook->is_noreg_table_exists())
{
...
"error",
" in <strong>".TrustindexPlugin::___('Widgets for Social Reviews & Recommendations')."</strong> plugin<br /><br />"
.TrustindexPlugin::___('Please fill out <strong>all the required fields</strong> in the <a href="%s">widget settings</a> page', [admin_url('admin.php?page='.$trustindex_pm_facebook->get_plugin_slug().'/settings.php')]),
false
);
}
echo $after_widget;
...
. TrustindexPlugin::___("Go to <a href='%s'>plugin setup page</a> to complete the one-step setup guide and enjoy the full functionalization!", array(admin_url('admin.php?page='.$trustindex_pm_facebook->get_plugin_slug().'/settings.php&tab=setup_trustindex')))
); ?>
<?php endif; ?>
</div>
<?php
