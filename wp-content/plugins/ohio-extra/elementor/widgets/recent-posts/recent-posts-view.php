	...
	<?php 
		...
		for ( $_post_i = $_post_start; $_post_i < $_post_end; $_post_i++ ) {
			...
			echo '<div class="' . $col_class . ' blog-post-masonry masonry-block post-offset ohio-card-wrapper';
				...
				$anim_attrs = '';
				if ( in_array( $settings['animation_type'], [ 'sync', 'async' ] ) ) {
					$anim_attrs .= ' data-aos-once="true"';
					$anim_attrs .= ' data-aos="' . $settings['animation_effect'] . '"';
					if ( $settings['animation_type'] == 'async' ) {
						$_delay = ( 400 / $settings['items_per_row_desktop'] ) * ( $_post_i % $settings['items_per_row_desktop'] );
						$_delay = (int) $_delay - ( $_delay % 50 );
						$anim_attrs .= ' data-aos-delay="' . $_delay . '"';
					}
