...
class OhioHelper {
	...
	*/
	...
	static public function projects_get_adjacent_post($_post, $direction = 'prev') {
		...
		$where = apply_filters( "get_{$adjacent}_post_where", $wpdb->prepare("WHERE p.post_date $op %s AND p.post_type = 'ohio_portfolio' AND p.ID <> %d AND p.post_status = 'publish'", $current_post_date, $_post->ID), $in_same_cat, $excluded_categories );
	...
	static public function projects_get_bounding_post($type = 'first') {
		...
		$join  = apply_filters( "get_{$adjacent}_post_join", '', $in_same_cat, $excluded_categories );
		$where = apply_filters( "get_{$adjacent}_post_where", "WHERE p.post_type = 'ohio_portfolio' AND p.post_status = 'publish'", $in_same_cat, $excluded_categories );
		$sort  = apply_filters( "get_{$adjacent}_post_sort", "ORDER BY p.post_date $order LIMIT 1" );
